...
Applications may extend the protocol for another body type.
Sequences and input streams can produce data as Busker reads them, as described in the next section.
...
Busker reads from that source and sends each part through a bounded response queue.
The queue limits pending response data, and the writer waits for space when the client cannot keep up.
Busker does not collect the whole body before the response starts (looking at you http-kit).
...
Busker exposes those parts of HTTP through a xref:api/ol-busker-protocols.adoc#ResponseEmitter[`ResponseEmitter`] for these HTTP features.

With an emitter, an application can:

* send zero or more informational responses with statuses from 100 through 199, except `101 Switching Protocols`;
* send `103 Early Hints` with `link` headers so the client can start preloading resources;
* send `102 Processing` while work continues;
* send one final status and header map, followed by body data over time;
* tell Busker when the response is complete; and
* run application cleanup through an `on-close` callback.

An informational response cannot contain a body.
Each informational response has its own complete header map.
After the final response is committed, its status and headers cannot change.
...
See the xref:api/ol-busker-protocols.adoc#ResponseEmitter[`ResponseEmitter` protocol reference] for the complete API and close behavior.
