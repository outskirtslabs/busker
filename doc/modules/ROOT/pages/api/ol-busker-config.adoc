...
The `:storage` key may be a xref:next@ol.clave::api/ol-clave-storage.adoc#Storage[`ol.clave.storage/Storage`] instance or a factory map.

When omitted, managed certificate automation uses xref:next@ol.clave::api/ol-clave-storage-file.adoc#file-storage[`ol.clave.storage.file/file-storage`] with its no-arg default root.
...
- `:factory` - Function or qualified symbol returning a xref:next@ol.clave::api/ol-clave-storage.adoc#Storage[`ol.clave.storage/Storage`] implementation (required).
- factory-specific keys - Options passed to the factory (default: none).
  For the built-in filesystem storage factory, see xref:next@ol.clave::api/ol-clave-storage-file.adoc#file-storage[`ol.clave.storage.file/file-storage`].
...
To write a custom solver, see xref:next@ol.clave::api/ol-clave-certificate.adoc[`ol.clave.certificate`] and
