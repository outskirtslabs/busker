...
pub fn build(b: *std.Build) void {
    ...
    const compile_flags_script = std.fmt.allocPrint(b.allocator,
        \\echo '# Auto-generated by Zig build - do not edit' > compile_flags.txt && \
        \\echo '-std=c17' >> compile_flags.txt && \
        \\echo '-Wall' >> compile_flags.txt && \
        \\echo '-Wextra' >> compile_flags.txt && \
        \\echo '-D_POSIX_C_SOURCE=200809L' >> compile_flags.txt && \
        \\echo '-DH2O_USE_LIBUV=0' >> compile_flags.txt && \
        \\echo '{s}' >> compile_flags.txt && \
        \\echo '{s}' >> compile_flags.txt && \
        \\echo '-I{s}/src/main/native' >> compile_flags.txt && \
        \\echo '-I{s}/../h2o-zig/zig-out/include' >> compile_flags.txt
...
fn buildShimForTarget(b: *std.Build, target_config: TargetConfig, optimize: std.builtin.OptimizeMode, use_boringssl: bool) void {
    ...
    if (use_boringssl) {
        if (b.lazyDependency("boringssl", .{
            ...
            shim.addIncludePath(ssl_artifact.getEmittedIncludeTree());
        }
    } else {
    ...
    shim.installHeader(b.path("shim.h"), "shim.h");
}
