...
(defn- new-h2o-worker [_opts]
  ;; ol.h2o always executes Ring handlers on virtual threads; the native workers
...
(defn bench-by-spec
  ...
  (let [wrk-opts-vec (if (vector? wrk-opts) wrk-opts [wrk-opts])
        ...
        server-opts (u/or-defaults server-opts {:server-id :ol-h2o
                                                :port (u/rand-free-port)})
