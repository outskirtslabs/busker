...
(def ^:const dep-ol-h2o "ol.http-clj (local)")
(def ^:const dep-http-kit "http-kit v2.8.0")
(def ^:const dep-jetty "ring-jetty-adapter v1.11.0")
...
(defonce wrk-script-path_
  (delay
    (let [path "src/test/clojure/ol/h2o/benchmark/wrk-script.lua"]
...
(defn run-wrk
  ...
  (let [{:keys [out] :as version-r} (shell ["wrk" "--version"])
    ...
    (let [script-file (java.io.File/createTempFile "ol-h2o-wrk-script-" ".lua")]
